...
// Site logo shown top-left of the nav bar, next to (replacing, once
// uploaded) the plain-text "DEMO LEQTURE." wordmark. Unlike the hero
// image/player, this has no admin-tier restriction — both Super Admin
// and Customer Admin can upload or remove it, same as Image link cards.

const BYTES_KEY = "bytes";
const INFO_KEY = "info";
const MAX_BYTES = 3 * 1024 * 1024; // 3MB — logos are small, no need for the hero image's 5MB cap

export default async (req) => {
  const store = getStore({ name: "demoleqture-logo", consistency: "strong" });
  ...
  if (req.method === "POST") {
    ...
    if (file.size > MAX_BYTES) {
      return Response.json({ error: "logo image must be smaller than 3MB" }, { status: 400 });
    }
  ...
  // GET is public. ?meta=1 returns lightweight JSON (whether a logo
